...
STATELESS DESIGN: Worker has no memory between frames.
All projectile data is sent in each job, processed, and returned.
This allows any worker to handle the job (no worker affinity needed).
...
def reset_projectile_state():
    """No-op for stateless design. Kept for API compatibility."""
    pass


def handle_projectile_update_batch(job_data: dict, cached_grid, cached_dynamic_meshes, cached_dynamic_transforms) -> dict:
    """
    Handle PROJECTILE_UPDATE_BATCH job (STATELESS).

    Receives ALL projectile data from main thread, processes, returns updated state.
    Worker has NO memory between frames - any worker can handle this job.

    Input job_data:
        {
            "dt": float,                    # time step in seconds
            "game_time": float,             # current game time
            "projectiles": [                # ALL active projectiles (stateless)
                {
                    "client_id": int,       # for matching with main thread
                    "pos": (x, y, z),
                    "vel": (vx, vy, vz),
                    "gravity": float,
                    "end_time": float,      # absolute expiration time
    ...
    Returns:
        {
            "success": bool,
            "updated_projectiles": [        # projectiles still active after update
                {
                    "client_id": int,
                    "pos": (x, y, z),
                    "vel": (vx, vy, vz),
            ...
            "expired": [{"client_id": int}, ...],  # projectiles that expired
            "active_count": int,
            "calc_time_us": float,
            "rays_cast": int,
            "tris_tested": int,
    ...
    projectiles = job_data.get("projectiles", [])
    ...
    # Process projectiles (stateless - just process what we received)
    updated_projectiles = []
    impacts = []
    expired = []
    rays_cast = 0
    ...
    for proj in projectiles:
        ...
        # Check expiration
        if game_time >= proj.get("end_time", 0):
            expired.append({"client_id": client_id})
            continue

        # Get current state
        pos = proj.get("pos", (0, 0, 0))
        vel = proj.get("vel", (0, 0, 0))
        px, py, pz = pos
        vx, vy, vz = vel
        g = proj.get("gravity", -9.8)
        ...
        if hit and proj.get("stop_on_contact", True):
            # Record impact
            impacts.append({
                "client_id": client_id,
                "owner_key": proj.get("owner_key", 0),
                "pos": hit_result["pos"],
            ...
            # Report final position at impact (not active)
        ...
        # Still active - report updated position
        active_count += 1
        updated_projectiles.append({
            "client_id": client_id,
    ...
    if rays_cast > 0 or impacts:
        logs.append(("PROJECTILE", f"UPDATE active={active_count} rays={rays_cast} tris={total_tris} impacts={len(impacts)} {calc_time_us:.0f}us"))
