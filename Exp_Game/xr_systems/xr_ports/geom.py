...
def queue_fwd_triplet_primary(kcc_sink, base_xyz, dir_xyz, step_len: float, r: float, h: float):
    """
    Enqueue XR 3-band union forward rays for the current sub-step (PRIMARY stage).
    Stashes result on kcc_sink._xr_fwd_primary.
    HUD (volatile): XR.fwd.req/ok/hit/lat_ms
    """
    import time as _t
    bx, by, bz = float(base_xyz[0]), float(base_xyz[1]), float(base_xyz[2])
    dx, dy, dz = float(dir_xyz[0]),  float(dir_xyz[1]),  float(dir_xyz[2])
    payload = {"origin": (bx, by, bz), "dir": (dx, dy, dz),
               "r": float(r), "h": float(h), "step_len": float(step_len)}

    t0 = _t.perf_counter()
    _hud_set("XR.fwd.req", int(1 + int(getattr(queue_fwd_triplet_primary, "_req", 0))), volatile=True)
    setattr(queue_fwd_triplet_primary, "_req", 1 + int(getattr(queue_fwd_triplet_primary, "_req", 0)))

    def _apply(res: dict):
        lat_ms = (_t.perf_counter() - t0) * 1000.0
        _hud_set("XR.fwd.ok",  int(1 + int(getattr(queue_fwd_triplet_primary, "_ok", 0))), volatile=True)
        setattr(queue_fwd_triplet_primary, "_ok", 1 + int(getattr(queue_fwd_triplet_primary, "_ok", 0)))
        _hud_set("XR.fwd.lat_ms", float(round(lat_ms, 2)), volatile=True)

        hit = bool(isinstance(res, dict) and res.get("hit", False))
        if hit:
            _hud_set("XR.fwd.hit", int(1 + int(getattr(queue_fwd_triplet_primary, "_hit", 0))), volatile=True)
            setattr(queue_fwd_triplet_primary, "_hit", 1 + int(getattr(queue_fwd_triplet_primary, "_hit", 0)))

        try:
            n = res.get("normal", (0.0, 0.0, 1.0)) if hit else (0.0, 0.0, 1.0)
            kcc_sink._xr_fwd_primary = {
                "t": _t.perf_counter(),
                "hit": hit,
                "raw": float(res.get("dist", 0.0)) if hit else 0.0,
                "normal": (float(n[0]), float(n[1]), float(n[2])),
                "src": str(res.get("src", "—")).upper() if hit else "—",
                "band": str(res.get("band", "—")) if hit else "—",
                "_frame_seq": (res.get("_frame_seq") if isinstance(res, dict) else None),
                "lat_ms": float(lat_ms),
            }
        except Exception:
        ...
        if _ok_to_print("_last_fwd_triplet_p", "dev_log_geom_hz", "dev_log_geom_console"):
            hs = "Y" if hit else "N"
            raw_txt = f"{kcc_sink._xr_fwd_primary.get('raw', 0.0):0.3f}" if hit else "—"
            src_txt = kcc_sink._xr_fwd_primary.get("src", "—") if hit else "—"
            band    = kcc_sink._xr_fwd_primary.get("band", "—") if hit else "—"
            seq_txt = (kcc_sink._xr_fwd_primary.get("_frame_seq", "—")
                       if isinstance(getattr(kcc_sink, "_xr_fwd_primary", None), dict) else "—")
            print(f"[FWD.TRIP] stage=PRIMARY ok hit={hs} src={src_txt} band={band} raw={raw_txt} lat={lat_ms:0.2f} ms seq={seq_txt}")

    xr_enqueue("geom.ray_triplet.v1", payload, _apply)


def queue_fwd_triplet_slide(kcc_sink, base_xyz, dir_xyz, step_len: float, r: float, h: float):
    """
    Enqueue XR 3-band union forward rays for the SLIDE stage.
    Stashes result on kcc_sink._xr_fwd_slide.
    HUD (volatile): XR.fwdS.* counters
    """
    import time as _t
    bx, by, bz = float(base_xyz[0]), float(base_xyz[1]), float(base_xyz[2])
    dx, dy, dz = float(dir_xyz[0]),  float(dir_xyz[1]),  float(dir_xyz[2])
    ...
    t0 = _t.perf_counter()
    _hud_set("XR.fwdS.req", int(1 + int(getattr(queue_fwd_triplet_slide, "_req", 0))), volatile=True)
    setattr(queue_fwd_triplet_slide, "_req", 1 + int(getattr(queue_fwd_triplet_slide, "_req", 0)))

    def _apply(res: dict):
        lat_ms = (_t.perf_counter() - t0) * 1000.0
        _hud_set("XR.fwdS.ok",  int(1 + int(getattr(queue_fwd_triplet_slide, "_ok", 0))), volatile=True)
        setattr(queue_fwd_triplet_slide, "_ok", 1 + int(getattr(queue_fwd_triplet_slide, "_ok", 0)))
        _hud_set("XR.fwdS.lat_ms", float(round(lat_ms, 2)), volatile=True)

        hit = bool(isinstance(res, dict) and res.get("hit", False))
        if hit:
            _hud_set("XR.fwdS.hit", int(1 + int(getattr(queue_fwd_triplet_slide, "_hit", 0))), volatile=True)
            setattr(queue_fwd_triplet_slide, "_hit", 1 + int(getattr(queue_fwd_triplet_slide, "_hit", 0)))

        try:
            n = res.get("normal", (0.0, 0.0, 1.0)) if hit else (0.0, 0.0, 1.0)
            kcc_sink._xr_fwd_slide = {
                "t": _t.perf_counter(),
                "hit": hit,
                "raw": float(res.get("dist", 0.0)) if hit else 0.0,
                "normal": (float(n[0]), float(n[1]), float(n[2])),
        ...
        if _ok_to_print("_last_fwd_triplet_s", "dev_log_geom_hz", "dev_log_geom_console"):
            hs = "Y" if hit else "N"
            raw_txt = f"{kcc_sink._xr_fwd_slide.get('raw', 0.0):0.3f}" if hit else "—"
            src_txt = kcc_sink._xr_fwd_slide.get("src", "—") if hit else "—"
            band    = kcc_sink._xr_fwd_slide.get("band", "—") if hit else "—"
            seq_txt = (kcc_sink._xr_fwd_slide.get("_frame_seq", "—")
                       if isinstance(getattr(kcc_sink, "_xr_fwd_slide", None), dict) else "—")
            print(f"[FWD.TRIP] stage=SLIDE ok hit={hs} src={src_txt} band={band} raw={raw_txt} lat={lat_ms:0.2f} ms seq={seq_txt}")
