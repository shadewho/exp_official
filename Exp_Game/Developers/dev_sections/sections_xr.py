# Exp_Game/Developers/dev_sections/sections_xr.py
...
class XRSection:
    key = "xr"
    column = "LEFT"      # ignored by auto layout
    order = 10
    prop_toggle = "dev_hud_show_xr"

    def measure(self, scene, STATE, BUS, scale, lh, width):
        if not getattr(scene, "dev_hud_show_xr", False):
            return 0
        lines = 0
        if getattr(scene, "dev_xr_core_status", True): lines += 1
        if getattr(scene, "dev_xr_core_flow",   True): lines += 1
        return lines * lh
    ...
    def draw(self, x, y, scene, STATE, BUS, scale, lh, width):
        ...
        if getattr(scene, "dev_xr_core_status", True):
            status = "✓ XR ALIVE" if xr_ok else "… XR Idle"
            draw_text(x, y, f"(1) {status}   rtt {rtt:.2f} ms   phase {ph:+.2f} ms", 12*scale); y -= lh
        if getattr(scene, "dev_xr_core_flow", True):
            draw_text(x, y, f"(2) XR Flow  req/s {rqhz:0.1f}  ok/s {okhz:0.1f}  fail/s {flhz:0.1f}", 12*scale); y -= lh
        return y
