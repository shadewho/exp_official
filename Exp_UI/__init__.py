...
if not _IS_WORKER_PROCESS:
    ...
    def unregister():
        # Remove all properties from bpy.types.Scene (safe deletion with checks)
        if hasattr(bpy.types.Scene, 'package_item_type'):
            del bpy.types.Scene.package_item_type
        if hasattr(bpy.types.Scene, 'package_sort_by'):
            del bpy.types.Scene.package_sort_by
        if hasattr(bpy.types.Scene, 'package_search_query'):
            del bpy.types.Scene.package_search_query
        if hasattr(bpy.types.Scene, 'my_addon_data'):
            del bpy.types.Scene.my_addon_data
        if hasattr(bpy.types.Scene, 'current_thumbnail_page'):
            del bpy.types.Scene.current_thumbnail_page
        if hasattr(bpy.types.Scene, 'total_thumbnail_pages'):
